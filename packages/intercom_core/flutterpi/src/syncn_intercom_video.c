...
//
// screen_w/screen_h come from Dart's view.physicalSize (video_decoder.dart)
// -- the WHOLE SCREEN's physical size, not the actual on-screen video
// widget's rendered size (Dart has no cheap way to measure the latter
// before the first frame). On a small panel (e.g. 480x480) that's already
// a reasonable proxy and this function scales down accordingly. On a
// large/landscape panel (confirmed on-device 2026-09-08: 1280x800 after
// this board's -r 90 rotation) it isn't: the source is only 1280x720, so
// fitting it inside a 1280x800 box computes right back to the full
// 1280x720 source with zero scaling, even though the call UI's actual
// video tile is far smaller than the whole screen. Full-res NV12->RGB
// shader conversion costs ~25-35ms of real GPU time per frame (measured
// on-device via this file's own on_new_sample timing log) on a GPU with a
// single hardware command queue shared with flutter-pi's own UI
// compositing -- confirmed via live kernel stack traces to periodically
// stall flutter-pi's raster thread in drm_atomic_helper_wait_for_vblanks
// for 300ms+ at a time (freezing the whole app, video included) whenever
// this queued shader work backs up behind a vblank deadline. The 480x480
// panel never hit this because its screen size alone already forced
...
static void compute_decode_target(int screen_w, int screen_h, int *out_w, int *out_h) {
    ...
    if (screen_w > SYNCN_INTERCOM_VIDEO_MAX_DECODE_W) screen_w = SYNCN_INTERCOM_VIDEO_MAX_DECODE_W;
    if (screen_h > SYNCN_INTERCOM_VIDEO_MAX_DECODE_H) screen_h = SYNCN_INTERCOM_VIDEO_MAX_DECODE_H;
