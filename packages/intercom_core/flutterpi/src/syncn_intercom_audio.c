...
static bool start_locked(struct syncn_intercom_audio *self, bool capture_enabled, bool headset_mode) {
    ...
    static const char *capture_desc_aec =
        // Channel-0 selection (2026-09-16): this panel's mic captures stereo
        // with channel 1 DEAD, so plain `audioconvert` averages a live
        // channel with silence and halves the signal before AGC ever sees
        // it. Measured on .203: rms 662 -> 1423 selecting channel 0 alone.
        // That is why AGC at target-level-dbfs=3 alone did not make the mic
        // audibly louder -- it cannot manufacture level from a signal that
        // was already halved upstream.
        //
        // Constrains ONLY channels=2, deliberately NOT the rate. v1.3.83's
        // version of this fix (c844ecd) pinned the full
        // rate=44100,channels=2 on plughw:0,0, and that full pinning is the
        // leading suspect for the silent-downlink regression on this
        // board's single shared PCM (capture claiming the device at a fixed
        // rate can stop the playback direction configuring it). Leaving the
        // rate free lets plughw negotiate as it does on the working
        // v1.3.73 base, so this takes the ~2x gain without the suspected
        // risk. If downlink goes silent again, THIS is the line to revert.
        "alsasrc device=plughw:0,0 ! audio/x-raw,channels=2 ! "
    ...
    static const char *capture_desc_plain =
        // Same channel-0 selection as the AEC variant above (dead channel 1
        // would otherwise halve the signal); rate left free for the same
        // downlink-safety reason.
        "alsasrc device=plughw:0,0 ! audio/x-raw,channels=2 ! "
        "deinterleave name=capdi  capdi.src_0 ! queue ! "
        "audioconvert ! audioresample quality=10 ! "
        "audio/x-raw,rate=8000,channels=1,format=S16LE ! "
