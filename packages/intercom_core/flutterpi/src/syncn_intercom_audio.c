...
// Blocking start: set_state(PLAYING) then confirms via gst_element_get_state()
// with a 3s timeout. Originally added for capture only; a blocking
// get_state() called directly from the platform thread had once deadlocked
// it (submit()/playDownlink() method calls couldn't reach appsrc while
// blocked -- see syncn_intercom_video.c's identical historical fix), so
// playback used to use a fire-and-forget non-blocking start instead.
//
// UPDATE (2026-09-16): that justification no longer applies to playback.
// start_locked() (which calls this) always runs on its own detached pthread
// (start_thread_main(), spawned via pthread_create() from the "start"
// method-channel handler), never on flutter-pi's platform thread -- so
// blocking here is safe from this call site, exactly as it already is for
// capture. The old non-blocking playback path silently treated
// GST_STATE_CHANGE_ASYNC as success without ever confirming the pipeline
// actually reached PLAYING, which let alsasink get stuck at ALSA state
// OPEN/hw_ptr=0 forever during real calls -- no bus error, no failure
// return, just permanent silence on the downlink. See
// docs/intercom-alsink-never-playing-2026-09-16.md for the live /proc/asound
// evidence. Now used for both capture and playback.
...
// Retries a blocking, confirmed pipeline start on failure, resetting to NULL
// and backing off briefly between attempts. Added after an on-device failure
...
static bool start_pipeline_with_retry(const char *tag, GstElement *pipeline, int max_attempts, guint retry_delay_ms) {
    for (int attempt = 1; attempt <= max_attempts; attempt++) {
        if (start_and_confirm_playing(tag, pipeline)) {
            if (attempt > 1) {
...
static bool start_locked(struct syncn_intercom_audio *self, bool capture_enabled, bool headset_mode) {
    ...
    // start_pipeline_with_retry() now blocks and confirms PLAYING on every
    // attempt (see its definition above) instead of firing-and-forgetting --
    // start_locked() runs on its own detached pthread (start_thread_main(),
    // spawned via pthread_create() in the "start" method-channel handler
    // below), never on flutter-pi's platform thread, so the old
    // platform-thread-deadlock risk that motivated a non-blocking start does
    // not apply from this call site. Capture already blocked safely the same
    // way. Confirmed root cause (2026-09-16): the old non-blocking start
    // treated GST_STATE_CHANGE_ASYNC as unconditional success and never
    // polled again, so alsasink could silently stay stuck at ALSA state
    // OPEN/hw_ptr=0 forever during a real call -- no bus error, no failure
    // return, just dead silence. See
    // docs/intercom-alsink-never-playing-2026-09-16.md.
