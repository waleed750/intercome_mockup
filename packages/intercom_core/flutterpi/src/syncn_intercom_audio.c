...
static bool start_locked(struct syncn_intercom_audio *self, bool capture_enabled, bool headset_mode) {
    ...
    // The `audiodynamic` compressor ahead of capvol is what actually makes
    // this mic usable at conversational distance, and it is NOT
    // interchangeable with more gain. Measured on real hardware
    // (192.168.100.203, 2026-09-15, speaking at normal standing distance,
    // 8kHz mono, raw capture with no DSP): peak=13056 (40% of full scale)
    // but rms=483 (1.5%) -- a peak-to-RMS ratio of ~27x, where normal
    // speech sits around 4-8x. The voice IS the 1.5%; the 40% peak is a
    // lone transient. That is why every previous attempt to fix this with
    // a bigger `volume` multiplier failed: flat gain hits the transient
    // ceiling and clips long before the body of the speech gets loud
    // enough, which is also the most likely real explanation for the
    // "loud whine/buzz" reported at capvol=1.8x back on 2026-09-09
    // (clipped transients, not an AEC interaction).
    //
    ...
    // (so the makeup gain applies to already-tamed peaks). Do not move it
    // past audiobuffersplit -- that element's position is what guarantees
    // the door's required 160 bytes/20ms uplink framing, see the long
    // comment above.
